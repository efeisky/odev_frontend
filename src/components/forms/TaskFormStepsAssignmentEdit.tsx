...
interface Props {
  ...
  onSubtaskAdded?: (subtask: SubtaskForm) => void;
  onSubtaskDeleted?: (subtaskId: number) => void;
  onSubtaskUserToggled?: (subtaskId: number, userId: string) => void;
}

export default function TaskFormStepsAssignmentEdit({
  unassigned_users,
  assigned_users,
  subtasks,
  loading = false,
  ...
  const filteredUsers = useMemo(() => {
    ...
    return unassigned_users.filter((u) =>
      u.name.toLowerCase().includes(term)
    );
  }, [searchTerm, unassigned_users]);
  ...
  const handleRemoveUser = (id: string) => {
    const user = assigned_users.find(u => u.code === id);
    if (user) onUserRemoved?.(user);
  };
  ...
  const handleAssignUser = (subtaskId: number, userId: string) => {
    onSubtaskUserToggled?.(subtaskId, userId);
  };
  ...
  return (
    <div className="flex flex-col gap-10 mt-6 max-w-4xl mx-auto">
      ...
      <div className="rounded-2xl shadow-md p-8 transition hover:shadow-lg">
        <h2 className="text-xl font-semibold mb-6">Göreve Atanmış Kullanıcılar</h2>
        ...
        {loading ? (
          ...
          <>
            <div className="relative mb-6 max-w-md">
              <input
                type="text"
                placeholder="Kullanıcı ekle... (en az 2 harf)"
                value={searchTerm}
      ...
      <div className="rounded-2xl shadow-md p-8 transition hover:shadow-lg">
        ...
        {subtasks.map((sub) => (
          <div key={sub.id} className="rounded-2xl p-5 transition hover:bg-gray-50">
            ...
            {assigned_users.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  ...
                  {assigned_users
                    .filter((u) => !sub.assignedUserIds.includes(u.code))
                    .map((u) => (
                      <option key={`${sub.id}-option-${u.code}`} value={u.code}>
                        {u.name}
                      </option>
                    ))}
                </select>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {sub.assignedUserIds.length > 0 ? (
                    sub.assignedUserIds.map((uid) => {
                      const user = assigned_users.find((u) => u.code === uid);
                      return (
                        <span
                          key={`${sub.id}-${uid}`}
